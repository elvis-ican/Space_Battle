...
public class Space {
    ...
    public Space(int width, int height) {
        this.width = width;
        this.height = height;
        checking = new Checking(width, height);
...
*/
    ...
    public static void main(String[] args) throws Exception {
        ...
        game.setShip(new Spaceship(25, 40));
        game.run();
    }
    ...
    public void run() throws Exception {
        ...
        while (ship.isAlive()) {
            ...
            canvas.clear();
            draw(canvas);
            checking.canvas.loadMatrix(canvas.getMatrix(), score, maxScore);
...
//        System.out.println("Game Over!");
    ...
    public void createUfo() {
        int k = 1;
        if (score >= 20 && score <=40) k = 2;
        if (score > 40 && score <=100) k = 3;
        if (score > 100 && score <=200) k = 4;
        if (score > 200 && score <=400) k = 5;
        if (score > 400) k = 6;
        if (ufos.size() > k) return;

        int random10 = (int) (Math.random() * 5);
        if (random10 == 0) {
